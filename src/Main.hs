...
import Dibujos.Escher (escherConfig)
import Dibujos.Efes (efesConfig)
import Dibujos.Efe(efeConf)
import Dibujos.Efe1(efeConf1)
import Dibujos.EfeEspe(efeEspe)
import Dibujos.EfeApi1(efeApi1)
...
configs :: [Conf]
configs = [feoConf, grillaConf, escherConfig, efesConfig,efeConf,efeConf1,efeEspe,efeApi1]
...
main = do
  ...
  when (length args > 2 || null args) $ do
    putStrLn "\nSólo puede elegir un dibujo. Para ver los dibujos use -list."
    exitFailure
  when (head args == "-list") $ do
    putStrLn "\nLos dibujos disponibles son:\n"
    mapM_ (putStrLn . name) configs
    putStrLn "\nEscribe el nombre del dibujo que quieras ver:"
    dib <- getLine
    initial' configs $ dib
    exitSuccess	
